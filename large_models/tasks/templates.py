...
class Template:
    ...
    Defines the interface for encoding inputs and verbalizing labels.
    """
    
    def encode(self, sample):
        """
        Encodes the input sample into a prompt string (without the answer/candidate).
        
        Args:
            sample (Sample): The data sample containing input fields.
            
        Returns:
            str: The prompted input.
        """
        raise NotImplementedError
    
    def verbalize(self, sample, candidate):
        """
        Appends the candidate (answer option) to the prompted input.
        
        Args:
            sample (Sample): The data sample.
            candidate (str): The specific candidate/option to append.
            
        Returns:
            str: The full text (prompt + candidate).
        """
        return candidate
    
    def encode_sfc(self, sample):
        """
        Encodes the sample for Surface Form Competition (SFC) calibration.
        Usually, this returns a context-free prompt (e.g., just "<mask>").
        """
        return "<mask>"
    
    def verbalize_sfc(self, sample, candidate):
        """
        Verbalizes the candidate for SFC calibration.
        """
        return candidate


class SST2Template(Template):
    """Template for the SST-2 Sentiment Analysis task."""
...
class CopaTemplate(Template):
    """Template for the COPA (Choice of Plausible Alternatives) task."""
    ...
    def get_prompt(self, sample):
        ...
        # Adjust capitalization based on settings
    ...
    def capitalize(self, c):
        if self.capitalization == "correct":
            words = c.split(" ")
            # Basic heuristic: lowercase the first word unless it's "I"
            if words[0] != "I":
...
class BoolQTemplate(Template):
    """Template for BoolQ (Boolean Question Answering)."""
    def encode(self, sample):
...
class BoolQTemplateV2(Template):
    """Alternative template for BoolQ with newlines."""
    def encode(self, sample):
        passage = sample.data["passage"]
        question = sample.data["question"]
        if not question.endswith("?"):
...
class BoolQTemplateV3(Template):
    """Alternative template for BoolQ with standard newlines."""
    def encode(self, sample):
        passage = sample.data["passage"]
        question = sample.data["question"]
        if not question.endswith("?"):
...
class CBTemplate(Template):
    """Template for CommitmentBank (CB)."""
...
class WICTemplate(Template):
    """Template for Word-in-Context (WiC)."""
    verbalizer = {0: "No", 1: "Yes"}
...
class WSCTemplate(Template):
    """Template for Winograd Schema Challenge (WSC)."""
    verbalizer = {0: "No", 1: "Yes"}
...
class ReCoRDTemplate(Template):
    """Template for ReCoRD (Reading Comprehension with Commonsense Reasoning)."""
...
class ReCoRDTemplateGPT3(Template):
    """GPT-3 style template for ReCoRD."""
...
class RTETemplate(Template):
    """Template for Recognizing Textual Entailment (RTE)."""
...
class SQuADv2Template(Template):
    """Template for SQuAD 2.0 (Generative task)."""
    ...
    def verbalize(self, sample, candidate):
        ...
        title = sample.data['title']
        context = sample.data['context']
        # For the prompt we only take the first answer if multiple exist
...
class DROPTemplate(Template):
    """Template for DROP (Discrete Reasoning Over Paragraphs)."""
