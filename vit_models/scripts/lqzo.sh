...
EXTRA_ZO_ARGS="--lozo_rank $LOZO_RANK --channel_scale $CHANNEL_SCALE --train_unquantized True"
if [ "$MOMENTUM" = "True" ]; then
    EXTRA_ZO_ARGS="$EXTRA_ZO_ARGS --momentum_lqzo"
fi

# Generate Experiment Tag
TAG=lqzo-${MODE}-rank${LOZO_RANK}-${TASK}-lr${LR}-eps${EPS}-bit${BIT}-seed${SEED}-new
